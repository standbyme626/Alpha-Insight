...
1. Week1-Week2 的“基础闭环 + 自修复”，对应本版阶段 A 的重点（真沙箱可用性与稳定执行）。
2. Week3 的“数值门禁 + 多模态报告”，对应本版阶段 B 的重点（统一结果对象与可追溯输出）。
3. Week4 的“扫描+告警+观测”，对应本版阶段 C 的重点（运营指标、失败聚类、阈值告警）。

执行策略：沿用旧版“周粒度推进”，但验收以本版 A/B/C 阶段门禁为准。
